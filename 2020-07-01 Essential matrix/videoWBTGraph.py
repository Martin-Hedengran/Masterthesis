...
def crop(img, cropvalue=2):
    height, width, fps = img.shape

    XBegin = int(width*(0.5-cropvalue))
    XEnd = int(width*(0.5+cropvalue))
    YBegin = int(height*(0.5-cropvalue))
    YEnd = int(height*(0.5+cropvalue))
...
for scaleIterator in range(3):
    ...
    if position2 == []:
        ...
        position.clear()
        rotation1to2.clear()
        translation1to2.clear()
        pgoPosition.clear()
        adjusted_position.clear()
        triang_points = []
        count = 0
        adjusted_R.clear()
        adjusted_T.clear()
    ...
    IMG_SCALE = 10 + IMG_SCALE
    CROP_VALUE = CROP_VALUE + 1


path = generate_path(position, startPos)
path2 = generate_path(position2, startPos)
path3 = generate_path(position3, startPos)
...
ax2.plot(path[:,2], path[:,0], label='60% Crop')
ax2.plot(path2[:,2], path2[:,0], label='40% Crop')
ax2.plot(path3[:,2], path3[:,0], label='20% Crop')
ax2.legend(loc=0)
#ax2.axis('equal')
ax2.set_title('Visual Odometry - image cropping')
